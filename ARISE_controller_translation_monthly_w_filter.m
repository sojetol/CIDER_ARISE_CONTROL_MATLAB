...
function [q,log_array] = ARISE_controller_translation_monthly_w_filter(outvals,log_array,refvals,preset_noise)
...
if length(size_outvals) ==2 ||size_outvals(3)<=12
    summing_time = 0;
    preset_noise = 0*preset_noise;
else
    summing_time = 11;
        % preset_noise = 0*preset_noise;
end
...
if firsttime == 1
    ...
    timestamp = loglines(end, 1) + 1/12;
    sumdt0 = loglines(end, 3) + (T0 - refvals(1))/12;
    sumdt1 = loglines(end, 5) + (T1 - refvals(2))/12;
    sumdt2 = loglines(end, 7) + (T2 - refvals(3))/12;

...
pdimchange = 24*60*60;
% Q_60N_matrix = pdimchange*ncread("u-cm174_pr.nc","pr_delta");
% Q_30N_matrix = pdimchange*ncread("u-cd297_pr.nc","pr_delta");
% Q_15N_matrix = pdimchange*ncread("u-cd352_pr.nc","pr_delta");
% Q_0N_matrix  = pdimchange*ncread("u-ch622_pr.nc","pr_delta");
% Q_15S_matrix = pdimchange*ncread("u-cd353_pr.nc","pr_delta");
% Q_30S_matrix = pdimchange*ncread("u-cd354_pr.nc","pr_delta");
% Q_60S_matrix = pdimchange*ncread("u-cm175_pr.nc","pr_delta");

ssp245_Q_data = processRun('QFLX',pdimchange,'SSP245','203501-206912.nc',[2015 2069],[1 2 3]);
% Q_SSP245_matrix = pdimchange*regridUKESMtoCESM((ncread("pr_ssp_ukesm_1.nc","pr")+ncread("pr_ssp_ukesm_2.nc","pr")+ncread("pr_ssp_ukesm_3.nc","pr")+ncread("pr_ssp_ukesm_4.nc","pr")+ncread("pr_ssp_ukesm_5.nc","pr"))/5);
...
Q_base = globalMean(Q_SSP245_matrix_base);
Q_60N_matrix = pdimchange*readSinglePoint(60,'N',1,'QFLX')-Q_SSP245_matrix- Q_SSP245_matrix_base;
Q_30N_matrix = mean(pdimchange*readSinglePoint(30,'N',[1 2 3],'QFLX'),4)-Q_SSP245_matrix- Q_SSP245_matrix_base;
Q_15N_matrix = mean(pdimchange*readSinglePoint(15,'N',[1 2 3],'QFLX'),4)-Q_SSP245_matrix- Q_SSP245_matrix_base;
Q_0N_matrix  = mean(pdimchange*readSinglePoint( 0,'N',[1 2 3],'QFLX'),4)-Q_SSP245_matrix- Q_SSP245_matrix_base;
Q_15S_matrix = mean(pdimchange*readSinglePoint(15,'S',[1 2 3],'QFLX'),4)-Q_SSP245_matrix- Q_SSP245_matrix_base;
Q_30S_matrix = mean(pdimchange*readSinglePoint(30,'S',[1 2 3],'QFLX'),4)-Q_SSP245_matrix- Q_SSP245_matrix_base;
Q_60S_matrix = pdimchange*readSinglePoint(60,'S',1,'QFLX')-Q_SSP245_matrix- Q_SSP245_matrix_base;
...
% AOD_60NS_fdbk = feedback_30_AOD_data.ensemble_monthly_average - AOD_base;
% 
...
default_Q_data = processRun('QFLX',24*60*60,'GAUSS-DEFAULT','203501-206912.nc',[2035 2069],[1 2 3]);
lower_5_Q_data = processRun('QFLX',24*60*60,'GAUSS-LOWER-0.5','203501-206912.nc',[2035 2069],[1 2 3]);
lower_1_Q_data = processRun('QFLX',24*60*60,'GAUSS-LOWER-1.0','203501-206912.nc',[2035 2069],[1 2 3]);
feedback_60_Q_data = processRun('QFLX',24*60*60,'GAUSS-60N_60S-LOWER-0.5','203501-206912.nc',[2035 2069],[1 2 3]);
feedback_30_Q_data = processRun('QFLX',24*60*60,'GAUSS-30N_30S-LOWER-0.5','203501-206912.nc',[2035 2069],[1 2 3]);
feedback_15_Q_data = processRun('QFLX',24*60*60,'GAUSS-15N_15S-LOWER-0.5','203501-206912.nc',[2035 2069],[1 2 3]);
feedback_0N_Q_data = processRun('QFLX',24*60*60,'GAUSS-0N-LOWER-0.5','203501-206912.nc',[2035 2069],[1 2 3]);
% Q_base = mean(ssp245_Q_data.ensemble_monthly_average);
ssp245_Q_data_above_base = ssp245_Q_data.ensemble_monthly_average - Q_base;
default_Q_data_above_base = default_Q_data.ensemble_monthly_average - Q_base;
lower_5_Q_data_above_base = lower_5_Q_data.ensemble_monthly_average - Q_base;
Q_EQ_fdbk = feedback_0N_Q_data.ensemble_monthly_average - Q_base;
Q_15NS_fdbk = feedback_15_Q_data.ensemble_monthly_average - Q_base;
Q_30NS_fdbk = feedback_30_Q_data.ensemble_monthly_average - Q_base;
Q_60NS_fdbk = feedback_60_Q_data.ensemble_monthly_average - Q_base;
...
% lower_5_T_data = processRun('TREFHT',1,'GAUSS-LOWER-0.5','203501-206912.nc',[2035 2069],[1 2 3]);
% lower_1_T_data = processRun('TREFHT',1,'GAUSS-LOWER-1.0','203501-206912.nc',[2035 2069],[1 2 3]);
ssp245_Q_data = processRun('QFLX',24*60*60,'SSP245','203501-206912.nc',[2035 2069],[1 2 3]);
default_Q_data = processRun('QFLX',24*60*60,'GAUSS-DEFAULT','203501-206912.nc',[2035 2069],[1 2 3]);
lower_5_Q_data = processRun('QFLX',24*60*60,'GAUSS-LOWER-0.5','203501-206912.nc',[2035 2069],[1 2 3]);
lower_1_Q_data = processRun('QFLX',24*60*60,'GAUSS-LOWER-1.0','203501-206912.nc',[2035 2069],[1 2 3]);
...
% figure
tiley = tiledlayout(1,3);
set(gcf, 'Position', [200, 200, 1200,400]) % Set figure size
tiley.Padding = 'tight';
%% Plot single point AOD
sp_legend = ["SSP2-4.5"; "60°N"; "30°N"; "15°N"; "0°N";"15°S";"30°S";"60°S"];
% my_tile = nexttile(1);
% hold on
% plot(ann_time, ssp245_AOD_data.ensemble_annual_average,"LineWidth",2,"Color",'k')
...
% plot(ann_time, averageEvery2d(12,1,AOD_60S+AOD_base),"LineWidth",2,"Color",colors(7,:))
% for i = 1:7
%     if i == 1
%         injection_12_60N = [];
%         for j = 1:35
...
my_tile = nexttile(1);
hold on
plot(ann_time, ssp245_Q_data.ensemble_annual_average,"LineWidth",2,"Color",'k')
plot(ann_time, averageEvery2d(12,1,Q_60N+Q_SSP245)+Q_base,"LineWidth",2,"Color",colors(1,:))
plot(ann_time, averageEvery2d(12,1,Q_30N+Q_SSP245)+Q_base,"LineWidth",2,"Color",colors(2,:))
plot(ann_time, averageEvery2d(12,1,Q_15N+Q_SSP245)+Q_base,"LineWidth",2,"Color",colors(3,:))
plot(ann_time, averageEvery2d(12,1,Q_0N+Q_SSP245)+Q_base,"LineWidth",2,"Color",colors(4,:))
plot(ann_time, averageEvery2d(12,1,Q_15S+Q_SSP245)+Q_base,"LineWidth",2,"Color",colors(5,:))
plot(ann_time, averageEvery2d(12,1,Q_30S+Q_SSP245)+Q_base,"LineWidth",2,"Color",colors(6,:))
plot(ann_time, averageEvery2d(12,1,Q_60S+Q_SSP245)+Q_base,"LineWidth",2,"Color",colors(7,:))
for i = 1:8
    if i == 1
        injection_12_60N = [];
        for j = 1:35
    ...
    all_injection_and_CO2 = [injections CO2_forcing_SSP245_month];
    Q_emu_matrix = CIDER_pattern_from_all_injections_and_CO2(all_injection_and_CO2,param_AOD_all,param_Q_all,pattern_Q_all);
    Q_emu = globalMean(Q_emu_matrix);
    plot(ann_time, averageEvery2d(12,1,Q_emu)+Q_base,"LineWidth",2,"Color",color,"LineStyle","--")
end
hold off
legend(sp_legend,"Location","nw")
xlabel("Year")
...
% hold on
% plot(ann_time, ssp245_AOD_data.ensemble_annual_average,"LineWidth",2,"Color",'k')
% plot(ann_time, averageEvery2d(12,1,AOD_EQ_fdbk+AOD_base),"LineWidth",2,"Color",colors(7,:))
% plot(ann_time, averageEvery2d(12,1,AOD_15NS_fdbk+AOD_base),"LineWidth",2,"Color",colors(8,:))
% plot(ann_time, averageEvery2d(12,1,AOD_30NS_fdbk+AOD_base),"LineWidth",2,"Color",colors(9,:))
% plot(ann_time, averageEvery2d(12,1,AOD_60NS_fdbk+AOD_base),"LineWidth",2,"Color",colors(10,:))
% 
...
% 
% plot(ann_time, averageEvery2d(12,1,AOD_fdbk_EQ+AOD_base),"LineWidth",2,"Color",colors(7,:),"LineStyle","--")
% plot(ann_time, averageEvery2d(12,1,AOD_fdbk_15NS+AOD_base),"LineWidth",2,"Color",colors(8,:),"LineStyle","--")
% plot(ann_time, averageEvery2d(12,1,AOD_fdbk_30NS+AOD_base),"LineWidth",2,"Color",colors(9,:),"LineStyle","--")
% % plot(ann_time, averageEvery2d(12,1,AOD_fdbk_60NS+AOD_base),"LineWidth",2,"Color",colors(10,:),"LineStyle","--")
% 
% hold off
...
% hold on
% plot(ann_time, ssp245_T_data.ensemble_annual_average,"LineWidth",2,"Color",'k')
% plot(ann_time, averageEvery2d(12,1,T_EQ_fdbk(1:end)+T_base),"LineWidth",2,"Color",colors(7,:))
% plot(ann_time, averageEvery2d(12,1,T_15NS_fdbk(1:end)+T_base),"LineWidth",2,"Color",colors(8,:))
% plot(ann_time, averageEvery2d(12,1,T_30NS_fdbk(1:end)+T_base),"LineWidth",2,"Color",colors(9,:))
% % plot(ann_time, averageEvery2d(12,1,T_60NS_fdbk(1:end)+T_base),"LineWidth",2,"Color",colors(10,:))
% 
...
% plot(ann_time, averageEvery2d(12,1,T_emu_fdbk_EQ+T_base),"LineWidth",2,"Color",colors(7,:),"LineStyle","--")
% plot(ann_time, averageEvery2d(12,1,T_emu_fdbk_15NS+T_base),"LineWidth",2,"Color",colors(8,:),"LineStyle","--")
% plot(ann_time, averageEvery2d(12,1,T_emu_fdbk_30NS+T_base),"LineWidth",2,"Color",colors(9,:),"LineStyle","--")
% % plot(ann_time, averageEvery2d(12,1,T_emu_fdbk_60NS+T_base),"LineWidth",2,"Color",colors(10,:),"LineStyle","--")
% 
% hold off
% % legend("SSP2-4.5","0°N Feedback for 1.0°C","15°N+15°S Feedback for 1.0°C","30°N+30°S Feedback for 1.0°C","60°N+60°S Feedback for 1.0°C","Location",'se')
% % xlabel("Year")
...
hold on
plot(ann_time, ssp245_Q_data.ensemble_annual_average,"LineWidth",2,"Color",'k')
plot(ann_time, averageEvery2d(12,1,Q_EQ_fdbk(1:end))+Q_base,"LineWidth",2,"Color",colors2(1,:))
plot(ann_time, averageEvery2d(12,1,Q_15NS_fdbk(1:end))+Q_base,"LineWidth",2,"Color",colors2(2,:))
plot(ann_time, averageEvery2d(12,1,Q_30NS_fdbk(1:end))+Q_base,"LineWidth",2,"Color",colors2(3,:))
plot(ann_time, averageEvery2d(12,1,Q_60NS_fdbk(1:end))+Q_base,"LineWidth",2,"Color",colors(4,:))

Q_emu_SSP245 = globalMean(CIDER_pattern_from_all_injections_and_CO2([0*injection_matrix_fdbk_0N CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));
Q_emu_fdbk_EQ = globalMean(CIDER_pattern_from_all_injections_and_CO2([injection_matrix_fdbk_0N CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));
Q_emu_fdbk_15NS = globalMean(CIDER_pattern_from_all_injections_and_CO2([injection_matrix_fdbk_15NS CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));
Q_emu_fdbk_30NS = globalMean(CIDER_pattern_from_all_injections_and_CO2([injection_matrix_fdbk_30NS CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));
Q_emu_fdbk_60NS = globalMean(CIDER_pattern_from_all_injections_and_CO2([injection_matrix_fdbk_60NS CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));

plot(ann_time, averageEvery2d(12,1,Q_emu_SSP245+Q_base),"LineWidth",2,"Color",'k',"LineStyle","--")
plot(ann_time, averageEvery2d(12,1,Q_emu_fdbk_EQ+Q_base),"LineWidth",2,"Color",colors2(1,:),"LineStyle","--")
plot(ann_time, averageEvery2d(12,1,Q_emu_fdbk_15NS+Q_base),"LineWidth",2,"Color",colors2(2,:),"LineStyle","--")
plot(ann_time, averageEvery2d(12,1,Q_emu_fdbk_30NS+Q_base),"LineWidth",2,"Color",colors2(3,:),"LineStyle","--")
plot(ann_time, averageEvery2d(12,1,Q_emu_fdbk_60NS+Q_base),"LineWidth",2,"Color",colors(4,:),"LineStyle","--")

hold off
legend("SSP2-4.5","0°N Feedback for 1.0°C","15°N+15°S Feedback for 1.0°C","30°N+30°S Feedback for 1.0°C","60°N+60°S Feedback for 1.0°C","Location",'nw')
% xlabel("Year")
ylabel("Evaporation (mm/day)")
xlabel("Year")
box on 
...
% hold on
% plot(ann_time,ssp245_AOD_data.ensemble_annual_average,"LineWidth",2,"Color",'k')
% plot(ann_time, default_AOD_data.ensemble_annual_average,"LineWidth",2,"Color",'b')
% plot(ann_time, lower_5_AOD_data.ensemble_annual_average,"LineWidth",2,"Color",'b')
% plot(ann_time, lower_1_AOD_data.ensemble_annual_average,"LineWidth",2,"Color",'b')
% AOD_fdbk_arise = globalMean(CIDER_AOD_pattern_from_all_injections(param_AOD_all,injection_default_monthly,pattern_AOD_all));
% AOD_fdbk_lower_5 = globalMean(CIDER_AOD_pattern_from_all_injections(param_AOD_all,injection_lower_5_monthly,pattern_AOD_all));
% AOD_fdbk_lower_1 = globalMean(CIDER_AOD_pattern_from_all_injections(param_AOD_all,injection_lower_1_monthly,pattern_AOD_all));
% plot(ann_time, averageEvery2d(12,1,AOD_fdbk_arise+AOD_base),"LineWidth",2,"Color",'b',"LineStyle","--")
% plot(ann_time, averageEvery2d(12,1,AOD_fdbk_lower_5+AOD_base),"LineWidth",2,"Color",'b',"LineStyle","--")
% plot(ann_time, averageEvery2d(12,1,AOD_fdbk_lower_1+AOD_base),"LineWidth",2,"Color",'b',"LineStyle","--")
% ylabel("AOD")
% xlabel("Year")
% hold off
...
hold on
plot(ann_time, ssp245_Q_data.ensemble_annual_average,"LineWidth",2,"Color",'k')
plot(ann_time, default_Q_data.ensemble_annual_average,"LineWidth",2,"Color",colors3(1,:))
plot(ann_time, lower_5_Q_data.ensemble_annual_average,"LineWidth",2,"Color",colors3(2,:))
plot(ann_time, lower_1_Q_data.ensemble_annual_average,"LineWidth",2,"Color",colors3(3,:))
plot(ann_time, averageEvery2d(12,1,Q_emu_SSP245+Q_base),"LineWidth",2,"Color",'k',"LineStyle","--")
Q_emu_arise = globalMean(CIDER_pattern_from_all_injections_and_CO2([injection_default_monthly CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));
Q_emu_lower_5 = globalMean(CIDER_pattern_from_all_injections_and_CO2([injection_lower_5_monthly CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));
Q_emu_lower_1 = globalMean(CIDER_pattern_from_all_injections_and_CO2([injection_lower_1_monthly CO2_forcing_SSP245_month], param_AOD_all,param_Q_all,pattern_Q_all));
plot(ann_time, averageEvery2d(12,1,Q_emu_arise+Q_base),"LineWidth",2,"Color",colors3(1,:),"LineStyle","--")
plot(ann_time, averageEvery2d(12,1,Q_emu_lower_5+Q_base),"LineWidth",2,"Color",colors3(2,:),"LineStyle","--")
plot(ann_time, averageEvery2d(12,1,Q_emu_lower_1+Q_base),"LineWidth",2,"Color",colors3(3,:),"LineStyle","--")

hold off
ylabel("Evaporation (mm/day)")
xlabel("Year")
title("(c)")
legend("SSP2-4.5","Multiobjective for 1.5°C","Multiobjective for 1.0°C","Multiobjective for 0.5°C","Location","nw")
my_tile.TitleHorizontalAlignment = 'left';

set(gcf,'renderer','painters')
print(gcf,'-dpng',["Uncoord_Emu_Paper/Uncoord_Plots/Figure_S1_" + getNow() + ".png"],'-r300')
function output_array = processSSP245(var,scaling,run_name,suffix,years,members,varargin)
...
if nargin>6
    ...
    SSP_time = (years(1):years(end))';
    SSP_M = readFromSSP45(var,scaling,run_name,suffix,years,members);
    SSP_annual_average = averageEvery(12,1,SSP_M);
    SSP_ensemble_monthly_average = averageAcrossEnsemble(SSP_M);
    SSP_ensemble_annual_average = averageAcrossEnsemble(SSP_annual_average);
    SSP_global_individual_annual_average = globalMean(SSP_annual_average);
    SSP_global_individual_monthly_average = globalMean(SSP_M);
    SSP_global_ensemble_annual_average = globalMean(SSP_ensemble_annual_average);
    SSP_global_ensemble_monthly_average = globalMean(SSP_ensemble_monthly_average);
    SSP_global_individual_annual_sum = globalSum(SSP_annual_average);
    SSP_global_ensemble_annual_sum = globalSum(SSP_ensemble_annual_average);
    SSP_global_ensemble_monthly_sum = globalSum(SSP_ensemble_monthly_average);
    run_global_individual_monthly_sum = globalSum(SSP_M);
end

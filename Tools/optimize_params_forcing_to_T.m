...
for k = 1:min(length(T_to_fit),length(SAI_forcing))
    T_emulated = 0;
    for j = 1:k
...
%     T_emulated = 0;
%     for j = 0:k-1
%         T_emulated = T_emulated + impulse_semiInfDiff(j,impulse_p_CO2)*CO2_forcing(k-j)+ impulse_semiInfDiff(j,impulse_p_SAI)*SAI_forcing(k-j);
%     end
%     diffs = diffs+(T_to_fit(k)-T_emulated)^2;
% end

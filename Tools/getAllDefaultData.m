function D = getAllDefaultData(var,scaling,run,suffix,years,ensemble_numbers,varargin)
...
if nargin>6
    ...
    D.seasonal_average = seasonal_average;
    D.seasonal_stdev = seasonal_stdev;
    year1 = max(years_array(1),years_array(end)-19);
    year2 = min(years_array(end),years_array(end));
end
...
if nargin>6
    ...
    ss_month_global_mean = globalMean(mean(averageByMonth(M_default,3),4));
    D.ss_global_mean = globalMean(ss_mean);
    ss_std_global = std(globalMean(M_in_a_row),0,1);
